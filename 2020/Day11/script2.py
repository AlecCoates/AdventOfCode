                    ...
                    for b in range(-1, 2):
                        if 0 <= i+a < len(floor_plan):
                            if 0<= j+b < len(floor_plan[i+a]):
                                if not (a == 0 and b == 0):
                                    c = 1
                                    while 0 <= i+a*c < len(floor_plan) and 0 <= j+b*c < len(floor_plan[i+a*c]):
                                        if floor_plan[i+a*c][j+b*c] == SEAT_OCCUPIED:
                                            adjacent_seats += 1
                                            break
                                        elif floor_plan[i+a*c][j+b*c] == SEAT_EMPTY:
                                            break
                                        c += 1
                if current_space == SEAT_EMPTY and adjacent_seats == 0:
                    new_floor_plan[i][j] = SEAT_OCCUPIED
                elif current_space == SEAT_OCCUPIED and adjacent_seats >= 5:
